...
- Items left for the user (not regressions, just operational notes from the audit doc): npm audit + depcheck not run in this read-only pass; ESLint is not configured in this repo; `CRON_SECRET` is intentionally absent from `.env.local` and replaced by the `x-vercel-cron` header path documented in the May 19 routines session.
- Net: every blocker the audit prompt calls out is resolved. No follow-up changes required for the platform to keep running.
