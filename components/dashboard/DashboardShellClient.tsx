...
/**
 * Everything the shell needs, resolved on the server by app/dashboard/layout.tsx.
 *
 * Null means the server read failed. The shell then falls back to the client
 * fetch path below, which is exactly how it behaved before this data was
 * server-rendered — so a failed server read costs a slower load, never a broken
 * dashboard.
 */
export interface DashboardShellInitialData {
...
export function DashboardShellClient({
  ...
  children: ReactNode
  initialData: DashboardShellInitialData | null
}) {
  return (
    <ToastProvider>
      <DashboardLayoutContent initialData={initialData}>{children}</DashboardLayoutContent>
    </ToastProvider>
  )
}
...
function DashboardLayoutContent({
  ...
  // Seeded from the server render. When that succeeded there is nothing to wait
  // for: the shell paints with real data on the first frame instead of
  // downloading, hydrating, and only then fetching /api/bootstrap.
  const [session, setSession] = useState<SessionData | null>(initialData?.session ?? null)
  const [projects, setProjects] = useState<ProjectItem[]>(initialData?.projects ?? [])
  const [ventures, setVentures] = useState<VentureItem[]>(initialData?.ventures ?? [])
  ...
  // ─── Data loading (fallback only) ─────────────────────────────────────────
  // The server render normally supplies all of this. This path runs only when
  // that read failed, and is byte-for-byte the behaviour the shell had before:
  // /api/bootstrap, degrading to the three original endpoints.
  const needsClientLoad = !initialData
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* Loading Screen — fallback path only.
          When the server supplied the shell data there is nothing to wait for,
          so this never mounts and the app is visible on the first frame. It
          still covers the case where the server read failed and the client has
          to fetch, where it dismisses as soon as that lands (200ms floor so a
          warm load doesn't flash). Left mounted once shown: it owns its own
          exit animation and the app fades in underneath it. */}
      {needsClientLoad && <LoadingScreen minimumDuration={200} ready={!loading} />}

      {/* No opacity gate. The whole app used to sit at opacity 0 until the
          client fetch resolved, which meant even a fully-rendered shell stayed
          invisible for the length of a network round-trip. */}
